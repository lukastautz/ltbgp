...
struct bgp_announcement_specs_s {
    ...
    uint32 *communities; // 32 bit per community, so sizeof(communities) = 4 * communities_count
    uint8 large_communities_count;
    uint32 *large_communities; // 96 bit per community, so sizeof(large_communities) = 12 * large_communities_count
